...
- Task text completion: When typing `rusk edit 3 <tab>`, automatically suggests the current task text for easy editing. (`rusk edit 3<tab>` will complete the ID). If the text contains shell-special characters (`| ; & > < ( ) [ ] { } $ " ' \` \ * ? ~ # @ ! % ^ = + - / : ,`), it is automatically wrapped in single quotes when completing (double quotes when text has `'`)
