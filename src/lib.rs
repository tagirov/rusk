...
/// Parse a due date from CLI input using today's local date as the base for relative values.
pub fn parse_cli_date(date_str: &str) -> Result<NaiveDate> {
    parse_cli_date_with_base(date_str, Local::now().date_naive())
}

/// Interactive date line: empty or whitespace-only means keep the current date; otherwise same rules as [`parse_cli_date`].
...
/// Parse a due date from CLI; `base` is used for relative expressions (e.g. `2w`, `1m3d`).
