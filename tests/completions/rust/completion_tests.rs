    ...
    let mut buffer: Vec<u8> = Vec::new();
    {
        ...
        for task in tasks {
            ...
            writeln!(
                stdout,
                "  {} {:>3} {:>10} {}",
                status, task.id, date_str, task.text
            )
            .unwrap();
        }
        writeln!(stdout).unwrap();
        ...
        if has_status && clean_line.contains(&task_id.to_string()) {
            ...
            if let Some(id_pos) = parts.iter().position(|&p| p == &task_id.to_string()) {
                ...
                if parts.len() > id_pos + 1 {
                    ...
                    let text_start =
                        if next_after_id.contains('-') && next_after_id.matches('-').count() == 2 {
...
fn test_completion_extract_task_ids() {
    ...
    tm.add_task(vec!["Task 2".to_string()], Some("01-01-2025".to_string()))
        .unwrap();
...
fn test_completion_extract_task_text_without_date() {
    ...
    tm.add_task(vec!["Simple task text".to_string()], None)
        .unwrap();
...
fn test_completion_format_stability() {
    ...
    tm.add_task(vec!["Undone no date".to_string()], None)
        .unwrap();
    tm.add_task(vec!["Done no date".to_string()], None).unwrap();
    tm.mark_tasks(vec![2]).unwrap();
    tm.add_task(
        vec!["Undone with date".to_string()],
        Some("15-06-2025".to_string()),
    )
    .unwrap();
    tm.add_task(
        vec!["Done with date".to_string()],
        Some("31-12-2025".to_string()),
    )
    .unwrap();
...
fn test_completion_awk_text_extraction() {
    ...
    tm.add_task(
        vec!["Multi word task text here".to_string()],
        Some("01-01-2025".to_string()),
    )
    .unwrap();
