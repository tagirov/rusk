...
fn test_completion_install(shell: Shell, expected_filename: &str) -> Result<()> {
    ...
    assert!(
        ...
        "Completion path should be a file"
    );
    ...
    assert!(
        completion_path.parent().unwrap().exists(),
        "Parent directory should exist"
    );
    assert!(
        completion_path.parent().unwrap().is_dir(),
...
fn test_all_shells_have_scripts() {
    for shell in [
        ...
        assert!(
            script.len() > 100,
            "Script for {:?} should be substantial",
            shell
        );
    }
}
...
fn test_completion_scripts_are_different() {
    ...
    assert_ne!(
        bash, powershell,
        "Bash and PowerShell scripts should differ"
    );
    ...
    assert_ne!(
        fish, powershell,
        "Fish and PowerShell scripts should differ"
    );
    ...
    for shell in [
        ...
        match shell {
            Shell::Bash => assert!(
                ...
                "Bash path should contain .bash_completion.d"
            ),
            Shell::Zsh => assert!(path_str.contains(".zsh"), "Zsh path should contain .zsh"),
            Shell::Fish => assert!(
                path_str.contains(".config/fish") || path_str.contains("fish"),
                "Fish path should contain fish"
            ),
            Shell::Nu => assert!(
                path_str.contains(".config/nushell") || path_str.contains("nushell"),
                "Nu path should contain nushell"
            ),
            Shell::PowerShell => assert!(
                path_str.contains("PowerShell") || path_str.contains("powershell"),
                "PowerShell path should contain PowerShell"
            ),
        }
    }
...
fn test_completion_instructions_are_provided() {
    ...
    for shell in [
        ...
        assert!(
            instructions.len() > 20,
            "Instructions for {:?} should be substantial",
            shell
        );
    }
}
...
fn test_completion_show_output() {
    ...
    for shell in [
        ...
        match shell {
            Shell::Bash => assert!(
                script.contains("#") || script.contains("complete"),
                "Bash script should contain shell syntax"
            ),
            Shell::Zsh => assert!(
                script.contains("#") || script.contains("_rusk"),
                "Zsh script should contain function definitions"
            ),
            Shell::Fish => assert!(
                script.contains("#") || script.contains("complete"),
            ...
            Shell::Nu => assert!(
                script.contains("#") || script.contains("def"),
                "Nu script should contain function definitions"
            ),
            Shell::PowerShell => assert!(
                script.contains("#")
                    || script.contains("Register-ArgumentCompleter")
                    || script.contains("function"),
                "PowerShell script should contain Register-ArgumentCompleter or function definitions"
            ),
        }
    }
}
...
fn test_completion_scripts_have_expected_structure() {
    ...
    let bash = Shell::Bash.get_script();
    assert!(
        bash.contains("rusk") || bash.contains("complete"),
        "Bash script should have completion structure"
    );

    let zsh = Shell::Zsh.get_script();
    assert!(
        zsh.contains("_rusk") || zsh.contains("function"),
        "Zsh script should have function definitions"
    );

    let fish = Shell::Fish.get_script();
    assert!(
        fish.contains("complete") || fish.contains("rusk"),
        "Fish script should have complete commands"
    );

    let nu = Shell::Nu.get_script();
    assert!(
        nu.contains("def") || nu.contains("rusk"),
        "Nu script should have function definitions"
    );

    let powershell = Shell::PowerShell.get_script();
    assert!(
        powershell.contains("Register-ArgumentCompleter") || powershell.contains("rusk"),
        "PowerShell script should have Register-ArgumentCompleter"
    );
}
...
fn test_cli_completions_show() -> Result<()> {
    ...
    for shell_name in ["bash", "zsh", "fish", "nu", "powershell"] {
        ...
        assert!(
            stdout.contains("rusk"),
            "Output for {} should contain 'rusk'",
            shell_name
        );
    }
...
fn test_cli_completions_install_overwrites_existing() -> Result<()> {
    ...
    assert_eq!(
        new_content,
        Shell::Fish.get_script(),
        "Content should match Fish script"
    );
...
fn test_cli_completions_install_multiple_shells() -> Result<()> {
    ...
    assert!(
        fish_path.is_file(),
        "Fish completion should exist under fake HOME"
    );
    assert!(
        nu_path.is_file(),
        "Nu completion should exist under fake HOME"
    );
...
fn test_cli_completions_help() -> Result<()> {
    ...
    assert!(
        ...
        "Help should mention 'completions'"
    );
...
fn test_completion_default_paths_live_under_home() -> Result<()> {
    ...
    assert!(
        real_bash_path.starts_with(&home),
        "Bash path should be in home"
    );
    assert!(
        real_zsh_path.starts_with(&home),
        "Zsh path should be in home"
    );
    assert!(
        real_fish_path.starts_with(&home),
        "Fish path should be in home"
    );
...
fn test_completion_scripts_match_source_files() -> Result<()> {
    ...
    assert_eq!(
        Shell::Bash.get_script(),
        source_bash,
        "Bash script should match source"
    );
    assert_eq!(
        Shell::Zsh.get_script(),
        source_zsh,
        "Zsh script should match source"
    );
    assert_eq!(
        Shell::Fish.get_script(),
        source_fish,
        "Fish script should match source"
    );
    assert_eq!(
        Shell::Nu.get_script(),
        source_nu,
        "Nu script should match source"
    );
    assert_eq!(
        Shell::PowerShell.get_script(),
        source_powershell,
        "PowerShell script should match source"
    );
...
fn test_nu_completion_has_quote_functions() {
    ...
    assert!(
        nu_script.contains("special_chars")
            || nu_script.contains("special")
            || nu_script.contains("|")
            || nu_script.contains(";"),
...
fn test_nu_completion_quotes_special_characters() {
    ...
    assert!(
        nu_script.contains("str replace")
            || nu_script.contains("replace")
            || nu_script.contains("\\\""),
...
fn test_nu_completion_completions_partial_input() {
    ...
    assert!(
        nu_script.contains("where")
            || nu_script.contains("filter")
            || nu_script.contains("matching"),
...
fn test_zsh_completion_syntax() -> Result<()> {
    ...
    let output = Command::new("zsh").arg("-n").arg(&script_path).output();
