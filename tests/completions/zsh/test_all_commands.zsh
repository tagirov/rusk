#!/bin/zsh
# Comprehensive tests for all rusk commands in Zsh
...
print_test_section "Zsh Completion Tests - All Commands"
...
# Test: Check if completion function exists
print_test "Completion function" "" "Should have _rusk function"
if (( $+functions[_rusk] )); then
    assert_true 0 "Completion function _rusk exists"
else
    assert_true 1 "Completion function _rusk exists"
fi
...
if [[ "$cnt" -ge 4 ]]; then
    ...
    assert_true 1 "Zsh script missing expected help branches (count=$cnt)"
fi
