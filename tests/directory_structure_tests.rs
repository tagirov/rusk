...
fn test_default_directory_structure() -> Result<()> {
    ...
    // Should create path in format: ~/.rusk/tasks.json
    assert!(db_path.to_string_lossy().contains("rusk"));
    assert!(db_path.file_name().unwrap() == "tasks.json");
    
    // Parent directory should be named ".rusk"
    let parent = db_path.parent().unwrap();
    assert!(parent.file_name().unwrap() == ".rusk");
...
fn test_nested_directory_structure() -> Result<()> {
    ...
    assert!(nested_path.parent().unwrap().parent().unwrap().parent().unwrap().exists());
...
fn test_get_db_dir_function() -> Result<()> {
    ...
    let expected_dir = temp_dir.path().join("subdir");
    
    assert_eq!(db_dir, expected_dir);
    
    // Test with directory path
    let custom_dir = temp_dir.path().join("another_dir");
    unsafe {
        env::set_var("RUSK_DB", format!("{}/", custom_dir.display()));
    }
    
    let db_dir = TaskManager::get_db_dir();
    assert_eq!(db_dir, custom_dir);
