...
fn test_edit_tasks_saves_only_when_changed() {
    ...
    let (_edited, unchanged, _not_found) = tm
        .edit_tasks(
            vec![1],
...
fn test_edit_tasks_date_parsing_validation() {
    ...
    let (_edited, _unchanged, _not_found) = tm
        .edit_tasks(vec![1], None, Some("31-12-2025".to_string()))
        .unwrap();
...
fn test_edit_tasks_comprehensive_scenario() {
    ...
    let expected_date = chrono::NaiveDate::parse_from_str("15-06-2025", "%d-%m-%Y").ok();
