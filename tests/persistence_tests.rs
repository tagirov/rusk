...
fn test_edit_tasks_persistence() {
    ...
    // Add a task
    tm.add_task(vec!["Original text".to_string()], None).unwrap();
    assert_eq!(tm.tasks().len(), 1);
    assert_eq!(tm.tasks()[0].text, "Original text");
    
    // Edit the task
    let (_edited, _unchanged, not_found) = tm.edit_tasks(vec![1], Some(vec!["New".to_string(), "text".to_string()]), None).unwrap();
    assert!(not_found.is_empty());
...
fn test_edit_nonexistent_task_no_save() {
    ...
    let (_edited, _unchanged, not_found) = tm.edit_tasks(vec![255], Some(vec!["New text".to_string()]), None).unwrap();
    assert_eq!(not_found, vec![255]);
